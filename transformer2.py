...
class CustomMultiheadAttention(nn.Module):  
    def __init__(self, embed_dim, num_heads, dropout=0.0, layer_index=None):  
    ...
    def forward(self, query, key, value, attn_mask=None, need_weights=False):  
        ...
        # 线性映射并拆分为多头  
        Q = self.q_linear(query).view(batch_size, seq_length, self.num_heads, self.head_dim).transpose(1, 2)  
        ...
        scaled_scores = scores / math.sqrt(self.head_dim)  # 缩放  
        if attn_mask is not None:  
            scaled_scores = scaled_scores.masked_fill(attn_mask == float('-inf'), float('-inf'))  
        temperature = 0.5  # 调整这个值来控制输出  
        attn_weights = F.softmax(scaled_scores / temperature, dim=-1)  

        # 计算注意力输出  
        attn_output = torch.matmul(attn_weights, V)  # 直接进行矩阵乘法  

        # 恢复形状  
        attn_output = attn_output.view(batch_size, self.num_heads, seq_length, self.head_dim)  
        attn_output = attn_output.transpose(1, 2).contiguous().view(batch_size, seq_length, embed_dim)  

        # 确保 output 被定义  
        output = self.out_proj(attn_output)  # 这里将 attn_output 赋值给 output  

        # 仅在翻译时打印 Q, K, V  
        if not self.training and self.layer_index == 0 and not self.printed:  
            self.print_attention_matrices(Q, K, V, scores, scaled_scores, attn_weights, attn_output, output)  
            self.printed = True  
    ...
    def print_attention_matrices(self, Q, K, V, scores, scaled_scores, attn_weights, attn_output, output):  
        # 取出第一个头部，打印第一个样本  
        Q = Q[0, 0]  # (seq_length, head_dim)  
        K = K[0, 0]  
        V = V[0, 0]  
        scores = scores[0, 0]  # (seq_length, seq_length)  
        scaled_scores = scaled_scores[0, 0]  
        attn_weights = attn_weights[0, 0]  
        attn_output = attn_output[0]  
        output = output[0]  # 取出投影后的输出矩阵  
        ...
        format_matrix(K, "K")  
        format_matrix(V, "V")  
        format_matrix(scores, "Scores")  
        format_matrix(scaled_scores, "Scaled Scores")  
        format_matrix(attn_weights, "Attention Weights")  
        format_matrix(attn_output, "Attention Output")  
        format_matrix(output, "Projected Output Matrix")  # 打印投影后的输出矩阵 

# Transformer 编码器层  
class CustomTransformerEncoderLayer(nn.Module):  
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, layer_index=0):  
        super(CustomTransformerEncoderLayer, self).__init__()  
        self.self_attn = CustomMultiheadAttention(d_model, nhead, dropout=dropout, layer_index=layer_index)  # 传递层的索引  
...
class TransformerModel(nn.Module):  
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=512, nhead=8, num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=2048, dropout=0.1):  
        ...
        encoder_layers = nn.ModuleList([  
            CustomTransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, layer_index=i)  
            for i in range(num_encoder_layers)  # 在这里传递层的索引  
        ])  
...
def train(model, data_loader, criterion, optimizer, num_epochs=100):  
    model.train() 
    # Reset the printed flag for each layer  
    for layer in model.encoder:  
        if isinstance(layer.self_attn, CustomMultiheadAttention):  
            layer.self_attn.printed = False  # Reset to False  
...
def translate(model, src_sentence, src_tokenizer, tgt_tokenizer):  
    ...
    # 重置每一层的 printed 标志  
    for layer_index, layer in enumerate(model.encoder):  
        if isinstance(layer.self_attn, CustomMultiheadAttention):  
            layer.self_attn.printed = False  # 重置为 False   
